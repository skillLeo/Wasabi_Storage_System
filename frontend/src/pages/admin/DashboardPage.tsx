...
function Avatar({ name, size = "md" }: { name: string; size?: "sm" | "md" }) {
  ...
  return (
    <div className={`${sz} rounded-full ${color} flex items-center justify-center text-white font-bold flex-shrink-0 shadow-sm`}>
      {initials}
    </div>
  );
}
...
export default function DashboardPage() {
  ...
  const statCards = [
    { label: "Total Employees", value: total, valueColor: "text-gray-900", iconBg: "bg-blue-50", iconColor: "text-blue-600",
      icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg> },
    { label: "Fully Complete", value: complete, valueColor: "text-emerald-600", iconBg: "bg-emerald-50", iconColor: "text-emerald-600",
      icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
    { label: "Incomplete", value: incomplete, valueColor: "text-amber-600", iconBg: "bg-amber-50", iconColor: "text-amber-600",
              ...
              {sorted.map((emp) => (
                <div key={emp.id} className="p-4">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    ...
                    <Link
                      to={`/admin/users/${emp.id}`}
                      className="flex-shrink-0 inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors"
                    >
                      View
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-xs font-bold ${emp.completion_percentage === 100 ? "text-emerald-600" : "text-blue-600"}`}>
                  ...
                  {emp.missing_slots.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {emp.missing_slots.slice(0, 2).map((s) => (
                        <span key={s} className="px-2 py-0.5 text-xs rounded-md bg-red-50 text-red-600 font-medium border border-red-100">{s}</span>
                      ))}
                      {emp.missing_slots.length > 2 && (
                        <span className="px-2 py-0.5 text-xs rounded-md bg-gray-100 text-gray-500 font-medium">+{emp.missing_slots.length - 2} more</span>
                      )}
                    </div>
                  )}
                </div>
                ...
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/70">
                    ...
                    <th className="px-6 py-3.5 text-left hidden lg:table-cell">
                      <span className="text-xs font-semibold uppercase tracking-wide text-gray-500">Missing Docs</span>
                    </th>
                    <th className="px-6 py-3.5 text-left hidden xl:table-cell">
                      <span className="text-xs font-semibold uppercase tracking-wide text-gray-500">Last Upload</span>
                    </th>
                    ...
                    <tr key={emp.id} className="hover:bg-blue-50/30 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          {!emp.is_active && (
                            <span className="px-2 py-0.5 text-xs rounded-full bg-gray-100 text-gray-400 font-medium">Inactive</span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 hidden lg:table-cell">
                        {emp.missing_slots.length === 0 ? (
                          ...
                          <div className="flex flex-wrap gap-1">
                            {emp.missing_slots.slice(0, 3).map((s) => (
                              <span key={s} className="px-2 py-0.5 text-xs rounded-md bg-red-50 text-red-600 font-medium border border-red-100">{s}</span>
                            ))}
                            {emp.missing_slots.length > 3 && (
                              <span className="px-2 py-0.5 text-xs rounded-md bg-gray-100 text-gray-500 font-medium">+{emp.missing_slots.length - 3} more</span>
                            )}
                          </div>
                        )}
                      </td>
                      <td className="px-6 py-4 hidden xl:table-cell">
                        <span className="text-xs text-gray-500">
                          {formatDate(emp.last_upload_at) ?? <span className="italic text-gray-300">No uploads yet</span>}
                        </span>
                      </td>
