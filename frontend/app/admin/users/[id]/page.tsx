...
export default function EmployeeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get(`/admin/users/${id}/slots`);
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-6 mb-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-0.5">
              ...
              {!employee.is_active && (
                <span className="px-2 py-0.5 text-xs rounded-full bg-gray-100 text-gray-500 font-medium border border-gray-200">Inactive</span>
              )}
            </div>
        ...
        <div className="mt-4">
          <ProgressBar percentage={employee.completion_percentage} />
        </div>
        ...
        {slots.map((slot) => (
          <div
            ...
            className={`bg-white rounded-2xl border shadow-sm overflow-hidden transition-all hover:shadow-md ${
              slot.status === "uploaded" ? "border-gray-100" : "border-dashed border-gray-200"
            }`}
          >
            <div className="p-5">
              <div className="flex items-start justify-between gap-2 mb-4">
                <div className="flex items-center gap-2.5 min-w-0">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${
                    slot.status === "uploaded" ? "bg-emerald-50" : "bg-gray-100"
                  }`}>
