...
function UploadCard({ slot }: { slot: SlotItem }) {
    ...
    function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        setUploading(true);
        router.post(`/employee/slots/${slot.slot_id}/upload`, fd, {
            onFinish: () => {
                setUploading(false);
            ...
            <div className="p-5 flex-1 flex flex-col">
                ...
                {slot.status === 'missing' && (
                    <p className="text-xs text-gray-400 mb-4 flex items-center gap-1.5">
                        <svg className="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
                ...
                <div className="mt-auto">
                    ...
                    <button onClick={() => fileRef.current?.click()} disabled={uploading}
                        ...
                        {uploading ? (
                            ...
                            <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>Replace File</>
                        ) : (
                            <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>Upload Document</>
                        )}
                    </button>
                </div>
            </div>
...
export default function Documents({ slots, completion_percentage, uploaded_slots, total_slots }: Props) {
    const { auth } = usePage().props as any;
    const firstName = (auth.user?.name as string | undefined)?.split(' ')[0] ?? 'there';
        ...
        <EmployeeLayout>
            <div className="mb-6">
                ...
                <p className="text-sm text-gray-500 mt-1">
                    {allDone ? 'All your documents are submitted — you\'re all set!' : 'Please upload your required documents below.'}
                </p>
            </div>

            <div className={`rounded-2xl border p-4 sm:p-6 mb-6 sm:mb-8 transition-all ${allDone ? 'bg-emerald-50 border-emerald-200 shadow-sm' : 'bg-white border-gray-100 shadow-sm'}`}>
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <p className={`text-sm sm:text-base font-bold ${allDone ? 'text-emerald-800' : 'text-gray-900'}`}>Document Progress</p>
                        <p className={`text-sm mt-0.5 ${allDone ? 'text-emerald-600' : 'text-gray-500'}`}>{uploaded_slots} of {total_slots} documents submitted</p>
                        {allDone && (
            ...
            {slots.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                    {slots.map((slot) => <UploadCard key={slot.id} slot={slot} />)}
                </div>
            )}
        </EmployeeLayout>
    );
